...
from .tools import fetch_what_entities_needs_to_be_collected, retrieve_relevant_context, retrieve_top_5_intents, validate_and_dump_collected_data_json, search_web, get_current_datetime, date_is_in_future
from .tools import fetch_in_what_structure_entities_needs_to_be_stored
from .structures import IntentPredictionOutput
from .prompts import ROOT_AGENT_PROMPT, KB_AGENT_PROMPT, INTENT_ENTITY_AGENT_PROMPT, INTENT_PREDICTION_AGENT_PROMPT



greeting_agent = LlmAgent(name="General_Chat_Agent",
...
#     You also has access to the current datetime function to get today's date and current time if you need it for some resolution.
#     """,
...
# action_data_restructure_agent = LlmAgent(name="Restructure_Data_Agent",  model="gemini-2.5-flash-preview-05-20",
#                                 generate_content_config=types.GenerateContentConfig(temperature=0.0),
...
action_executor_agent = Agent(
    ...
    instruction="""
    You have access to multiple tools, you can execute actions as per requirements. Just ask a confirmation from the user before proceeding.
    """,
    tools=[register_patient, submit_feedback]
)


intent_entity_agent = LlmAgent(name="Intent_Entity_Agent", 
...
kb_agent = LlmAgent(name="Knowledge_Base_Agent", 
                    model="gemini-2.0-flash-exp",
                    generate_content_config=types.GenerateContentConfig(temperature=0.0),
...
notification_agent = LlmAgent(name="Notification_Agent", 
                    generate_content_config=types.GenerateContentConfig(temperature=0.0),
                    description="Transform final responses into formats required for different channels as email/whatsapp and then sends it to user.")
